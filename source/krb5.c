...
static int ASN1CALL ASN1Enc_KERB_CRED(ASN1encoding_t enc, ASN1uint32_t tag, KERB_CRED *val)
{
    ...
    if (!MSASN1$ASN1BEREncExplicitTag(enc, tag ? tag : 0x40000016, &nExplTagLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x10, &nLenOff))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000000, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncS32(enc, 0x2, (val)->version))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000001, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncS32(enc, 0x2, (val)->message_type))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!ASN1Enc_KERB_CRED_tickets(enc, 0, &(val)->tickets))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000003, &nLenOff0))
        return 0;
    if (!ASN1Enc_KERB_ENCRYPTED_DATA(enc, 0, &(val)->encrypted_part))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nExplTagLenOff0))
        return 0;
    return 1;
}
...
static int ASN1CALL ASN1Dec_KERB_AP_REQUEST(ASN1decoding_t dec, ASN1uint32_t tag, KERB_AP_REQUEST *val)
{
    ...
    ASN1decoding_t dd0;
    ASN1octet_t *di0;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x4000000e, &pExplTagDec0, &pbExplTagDataEnd0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(pExplTagDec0, 0x10, &dd, &di))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->version))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->message_type))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000002, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecBitString(dd0, 0x3, &(val)->ap_options))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000003, &dd0, &di0))
        return 0;
    if (!ASN1Dec_KERB_TICKET(dd0, 0, &(val)->ticket))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000004, &dd0, &di0))
        return 0;
    if (!ASN1Dec_KERB_ENCRYPTED_DATA(dd0, 0, &(val)->authenticator))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(pExplTagDec0, dd, di))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, pExplTagDec0, pbExplTagDataEnd0))
        return 0;
    return 1;
}
...
static int ASN1CALL ASN1Enc_KERB_CRED_tickets(ASN1encoding_t enc, ASN1uint32_t tag, PKERB_CRED_tickets *val)
{
    ...
    ASN1uint32_t nLenOff;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, tag ? tag : 0x80000002, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x10, &nLenOff))
        return 0;
...
static int ASN1CALL ASN1Enc_KERB_TICKET(ASN1encoding_t enc, ASN1uint32_t tag, KERB_TICKET *val)
{
    ...
    ASN1uint32_t nLenOff0;
    ASN1uint32_t t;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, tag ? tag : 0x40000001, &nExplTagLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x10, &nLenOff))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000000, &nLenOff0))
    ...
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000001, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1DEREncCharString(enc, 0x1b, t, (val)->realm))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000002, &nLenOff0))
        return 0;
    ...
    if ((val)->o[0] & 0x80)
    {
        if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000004, &nLenOff0))
            return 0;
...
static int ASN1CALL ASN1Enc_KERB_PRINCIPAL_NAME(ASN1encoding_t enc, ASN1uint32_t tag, KERB_PRINCIPAL_NAME *val)
{
    ASN1uint32_t nLenOff;
    ASN1uint32_t nLenOff0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, tag ? tag : 0x10, &nLenOff))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000000, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncS32(enc, 0x2, (val)->name_type))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
                                                            ...
                                                            PKERB_PRINCIPAL_NAME_name_string *val)
{
    ...
    ASN1uint32_t nLenOff;
    ASN1uint32_t t;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, tag ? tag : 0x80000001, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x10, &nLenOff))
        return 0;
    for (f = *val; f; f = f->next)
    {
        t = _strlen(f->value);
        if (!MSASN1$ASN1DEREncCharString(enc, 0x1b, t, f->value))
            return 0;
    }
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff))
        return 0;
...
static int ASN1CALL ASN1Enc_KERB_ENCRYPTED_DATA(ASN1encoding_t enc, ASN1uint32_t tag, KERB_ENCRYPTED_DATA *val)
{
    ...
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000002, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1DEREncOctetString(enc, 0x4, ((val)->cipher_text).length, ((val)->cipher_text).value))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff))
                                                        ...
                                                        PKERB_TICKET_EXTENSIONS_Seq *val)
{
    ...
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000000, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncS32(enc, 0x2, (val)->te_type))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncExplicitTag(enc, 0x80000001, &nLenOff0))
        return 0;
    if (!MSASN1$ASN1DEREncOctetString(enc, 0x4, ((val)->te_data).length, ((val)->te_data).value))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff0))
        return 0;
    if (!MSASN1$ASN1BEREncEndOfContents(enc, nLenOff))
...
static int ASN1CALL ASN1Dec_KERB_TICKET(ASN1decoding_t dec, ASN1uint32_t tag, KERB_TICKET *val)
{
    ...
    ASN1octet_t *di0;
    ASN1uint32_t t;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x40000001, &pExplTagDec0, &pbExplTagDataEnd0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(pExplTagDec0, 0x10, &dd, &di))
        return 0;
    _memset((val)->o, 0, 1);
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecZeroCharString(dd0, 0x1b, &(val)->realm))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000002, &dd0, &di0))
...
static int ASN1CALL ASN1Dec_KERB_PRINCIPAL_NAME(ASN1decoding_t dec, ASN1uint32_t tag, KERB_PRINCIPAL_NAME *val)
{
    ...
    ASN1decoding_t dd0;
    ASN1octet_t *di0;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x10, &dd, &di))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->name_type))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!ASN1Dec_KERB_PRINCIPAL_NAME_name_string(dd, 0, &(val)->name_string))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd, di))
        return 0;
    return 1;
}
                                                            ...
                                                            PKERB_PRINCIPAL_NAME_name_string *val)
{
    ...
    ASN1octet_t *di;
    ASN1uint32_t t;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd0, 0x10, &dd, &di))
        return 0;
    f = val;
    while (MSASN1$ASN1BERDecNotEndOfContents(dd, di))
    {
        if (!MSASN1$ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PKERB_PRINCIPAL_NAME_name_string)MSASN1$ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
        if (!MSASN1$ASN1BERDecZeroCharString(dd, 0x1b, &(*f)->value))
            return 0;
        f = &(*f)->next;
    }
    *f = NULL;
    if (!MSASN1$ASN1BERDecEndOfContents(dd0, dd, di))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd0, di0))
        return 0;
    return 1;
}
...
static int ASN1CALL ASN1Dec_KERB_ENCRYPTED_DATA(ASN1decoding_t dec, ASN1uint32_t tag, KERB_ENCRYPTED_DATA *val)
{
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000002, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecOctetString(dd0, 0x4, &(val)->cipher_text))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd, di))
                                                    ...
                                                    PPKERB_TICKET_EXTENSIONS *val)
{
    ...
    while (MSASN1$ASN1BERDecNotEndOfContents(dd, di))
    {
        if (!MSASN1$ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PPKERB_TICKET_EXTENSIONS)MSASN1$ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
                                                        ...
                                                        PKERB_TICKET_EXTENSIONS_Seq *val)
{
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->te_type))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecOctetString(dd0, 0x4, &(val)->te_data))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd, di))
...
static int ASN1CALL ASN1Dec_KERB_AUTHENTICATOR(ASN1decoding_t dec, ASN1uint32_t tag, KERB_AUTHENTICATOR *val)
{
    ...
    ASN1octet_t *di0;
    ASN1uint32_t t;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x40000002, &pExplTagDec0, &pbExplTagDataEnd0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(pExplTagDec0, 0x10, &dd, &di))
        return 0;
    _memset((val)->o, 0, 1);
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->authenticator_version))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecZeroCharString(dd0, 0x1b, &(val)->client_realm))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000002, &dd0, &di0))
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000004, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->client_usec))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000005, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecGeneralizedTime(dd0, 0x18, &(val)->client_time))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    MSASN1$ASN1BERDecPeekTag(dd, &t);
    ...
    {
        (val)->o[0] |= 0x40;
        if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000006, &dd0, &di0))
            return 0;
    ...
    {
        (val)->o[0] |= 0x20;
        if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000007, &dd0, &di0))
            return 0;
        if (!MSASN1$ASN1BERDecSXVal(dd0, 0x2, &(val)->sequence_number))
            return 0;
        if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
            return 0;
    }
    ...
    {
        (val)->o[0] |= 0x10;
        if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000008, &dd0, &di0))
            return 0;
...
static int ASN1CALL ASN1Dec_KERB_CHECKSUM(ASN1decoding_t dec, ASN1uint32_t tag, KERB_CHECKSUM *val)
{
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->checksum_type))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecOctetString(dd0, 0x4, &(val)->checksum))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd, di))
...
static int ASN1CALL ASN1Dec_KERB_ENCRYPTION_KEY(ASN1decoding_t dec, ASN1uint32_t tag, KERB_ENCRYPTION_KEY *val)
{
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->keytype))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecOctetString(dd0, 0x4, &(val)->keyvalue))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd, di))
                                                     ...
                                                     PPKERB_AUTHORIZATION_DATA *val)
{
    ...
    while (MSASN1$ASN1BERDecNotEndOfContents(dd, di))
    {
        if (!MSASN1$ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PPKERB_AUTHORIZATION_DATA)MSASN1$ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
                                                         ...
                                                         PKERB_AUTHORIZATION_DATA_Seq *val)
{
    ...
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecS32Val(dd0, 0x2, &(val)->auth_data_type))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000001, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecOctetString(dd0, 0x4, &(val)->auth_data))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dd, dd0, di0))
        return 0;
    if (!MSASN1$ASN1BERDecEndOfContents(dec, dd, di))
...
static int ASN1CALL ASN1Dec_KERB_CRED(ASN1decoding_t dec, ASN1uint32_t tag, KERB_CRED *val)
{
    ...
    ASN1decoding_t dd0;
    ASN1octet_t *di0;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x40000016, &pExplTagDec0, &pbExplTagDataEnd0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(pExplTagDec0, 0x10, &dd, &di))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd, 0x80000000, &dd0, &di0))
...
static int ASN1CALL ASN1Dec_KERB_CRED_tickets(ASN1decoding_t dec, ASN1uint32_t tag, PKERB_CRED_tickets *val)
{
    ...
    ASN1octet_t *di;
    ASN1uint32_t t;
    if (!MSASN1$ASN1BERDecExplicitTag(dec, tag ? tag : 0x80000002, &dd0, &di0))
        return 0;
    if (!MSASN1$ASN1BERDecExplicitTag(dd0, 0x10, &dd, &di))
        return 0;
    f = val;
    while (MSASN1$ASN1BERDecNotEndOfContents(dd, di))
    {
        if (!MSASN1$ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PKERB_CRED_tickets)MSASN1$ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
